  ...
  pocControllers.controller('RoomController', ['$scope', '$http', '$location', '$routeParams', 'pocData', function($scope, $http, $location, $routeParams, pocData){
    ...
    $scope.initialize = function(flag){
      if(flag){
        ...
        $scope.resetIsEditing = function(){
         $scope.pocData.roomData.forEach(function(obj){
          obj.isEditing=false;
        })
       }

       $scope.clickRoom = function(room){
        var supressRoomClickEvent = false;
        $scope.pocData.roomData.forEach(function(obj){
          if(obj.isEditing) supressRoomClickEvent = true;
        });

        if(!supressRoomClickEvent){
          $scope.resetIsEditing();
     ...
     $scope.editRoom = function(index){
      var room = $scope.pocData.roomData[index];
      if(room.roomName.trim()=="" || room.address.trim()==""){
        $scope.error = "Please enter both Room and Address details"
      }
      else{
        $scope.error = null;
        $scope.pocData.roomData[index].isEditing =  !$scope.pocData.roomData[index].isEditing; 
      }
    };
    ...
    $scope.initialize = function(flag){
      if(flag){
        ...
        $scope.clickItem = function(item){
         var supressLevel4ClickEvent = false;
         $scope.pocData.levelFourData.forEach(function(obj){
          if(obj.isEditing) supressLevel4ClickEvent = true;
        });

         if(!supressLevel4ClickEvent){
           $scope.resetIsEditing();
     ...
     $scope.editItem = function(index){
      var level4 = $scope.pocData.levelFourData[index];
      if(level4.levelFourName.trim()=="" || level4.levelFourAddress.trim()==""){
        $scope.error = "Please enter both Level and Address details"
      }
      else{
        $scope.error = null;
        $scope.pocData.levelFourData[index].isEditing =  !$scope.pocData.levelFourData[index].isEditing; 
      }
    };
  ...
  $scope.editItem = function(index){
      var level5 = $scope.pocData.levelFiveData[index];
      if(level5.levelFiveName.trim()=="" || level5.levelFiveAddress.trim()==""){
        $scope.error = "Please enter both Level and Address details"
      }
      else{
        $scope.error = null;
      $scope.pocData.levelFiveData[index].isEditing =  !$scope.pocData.levelFiveData[index].isEditing; 
    }
    };
